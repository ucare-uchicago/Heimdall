...
char io_percentage_list_str[600];
...
*/
int random_algo() {
    if (rand() % 100 > 50) {     // 50% possibility for each device
        return (original_device_index + 1) % total_dev_num;
    }
...
int main(int argc, char **argv) {
    ...
    if (argc != 6) {
        printf("Usage: ./io_replayer $original_device_index $device_list $trace $output_file $duration\n");
        exit(1);
    } else {
