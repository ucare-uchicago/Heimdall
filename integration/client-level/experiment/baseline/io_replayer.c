...
void *pr_progress() {
    ...
    while (!shouldQuit()) {
        ...
        // It is difficult to predict the number of IOs that will be submitted 
        // So the progress rate is inaccurate
        // We just divide the current number of I/Os replayed by total I/Os 
        //      or by number of I/Os replayed if it is bigger than number of total I/Os
...
int main(int argc, char **argv) {
    ...
    if (argc != 5) {
        printf("Usage: ./io_replayer /dev/nvme0n1 tracefile output_file duration\n");
        exit(1);
    } else {
