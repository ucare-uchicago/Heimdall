...
/*
    A client implementation of flashnet
*/
int flashnet_algo(long io_type, long size, long cur_queue_len) {
    int target_device = original_device_index;
    
    int prediction_result = flashnet_inference(io_type, size, target_device, cur_queue_len);   //input arguments: io_type, size, device
    if (io_type == READ_IO) {
        ...
        printf("Flashnet Only Applied to Read IO\n");
        exit(1);
    }
...
void *perform_io() {
    ...
    while (!shouldQuit()) {
        ...
        if (reqflag[cur_idx] == READ_IO){
            target_device = flashnet_algo((long)reqflag[cur_idx], (long)reqsize[cur_idx], cur_queue_len);  // io_type, size, currrent_queue_length
        }
        ...
        // Need to update the flashnet, if the IO still goes to the original device
        ...
        for (int i = complete_idx; i <= cur_idx; i++) {
            ...
            if (req_queue_len[i] != IO_NOT_COMPLETED && req_latency[i] != IO_NOT_COMPLETED && req_throughput[i] != IO_NOT_COMPLETED) {
                ...
                if (req_queue_len[i] >= 0 && req_latency[i] >= 0 && req_throughput[i] >= 0) {
                    ...
                    update_flashnet(req_queue_len[i], req_latency[i], req_throughput[i]);
