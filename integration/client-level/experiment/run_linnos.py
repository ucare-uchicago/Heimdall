...
ALGORITHM = "linnos"
Inflection_percentile = 90 # We are not using this, the IP will be calculated dynamically based on the baseline trace
...
def make_linnos(trace_dir: str, devices_list: List[str], specific_workplace: str) -> bool:
    ...
    # 1.3. delete the weights folder (otherwise, it may cause expected interference in multiple experiments)
    return True


def train_linnos(trace_dir: str, devices: List[str]) -> bool:
    '''
        Input:
            `trace_dir`: path of a directory, which stores the result of baseline.
            `devices`: e.g. ['/dev/nvme0n1', '/dev/nvme2n1']
        Perform:
            Train linnos model based on baseline's result.
    ...
    train_command = ["./train_linnos.sh"]
    # 2. format devices
    for dev in dev_names:
        train_command.append(dev)
    # 3. format Inflection_percentile
    train_command.append(str(Inflection_percentile))
    # 4. format input dir
...
if __name__ == '__main__':
    ...
    for idx, trace_dir in enumerate(trace_dirs):
        ...
        output_eval_stats_0 = os.path.join(output_dir, "training_results", "mldrive0", "eval.stats")
        output_eval_stats_1 = os.path.join(output_dir, "training_results", "mldrive1", "eval.stats")

        if args.resume: 
            # check if the trace is already replayed
            if os.path.isfile(output_stat_path) and os.path.isfile(output_weights_path_0) and os.path.isfile(output_weights_path_1) and os.path.isfile(output_eval_stats_0) and os.path.isfile(output_eval_stats_1):
        ...
        if not args.only_replaying:
            ...
            if args.resume: 
                ...
                if  os.path.isfile(output_weights_path_0) and os.path.isfile(output_weights_path_1) and os.path.isfile(output_eval_stats_0) and os.path.isfile(output_eval_stats_1):
                    print("     The model is trained, skipping\n\n")
                    continue

            # 1. train linnos
            train_result = train_linnos(trace_dir, args.devices)
            if train_result == False:   # Fail in training
                print("\n[Train Linnos Error!!!!], dir: {}".format(trace_dir))
                exit(-1)
