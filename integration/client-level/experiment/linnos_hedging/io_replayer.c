...
void parse_io(char **reqs, int total_io) {
    ...
    if (oft == NULL || reqsize == NULL || reqflag == NULL ||
        timestamp == NULL || submission_ts_list == NULL || io_finish_status == NULL) {
    ...
    for (i = 0; i < total_io; i++) {
        ...
        // 6. init all the IO's submission time to be -1 (not-valid) first
        submission_ts_list[i] = -1;

        // 7. init all IOs to be not finished
        io_finish_status[i] = IO_NOT_FINISH;

        // printf("%.2f,%ld,%d,%d,%f,%d\n", timestamp[i], oft[i], reqsize[i], reqflag[i], submission_ts_list[i], io_finish_status[i]);
        // exit(1);
    }
...
void hedging_thread_function(int64_t cur_idx) {
    ...
    int status = io_finish_status[cur_idx];
    if (status == IO_FINISH) { 
        return;
    }
...
/*
    A client implementation of linnos
*/
int linnos_algo(long io_type, long size) {
    int target_device = original_device_index;
    if (io_type == READ_IO) {
        long input_size = ((size / block_size) / 512 + 7) / 8;        // align with the transform in training part.
        int prediction_result = linnos_inference(input_size, target_device);   //input arguments: io_type, size, device
        ...
        printf("Linnos Algorithm Only Applied to Read IO\n");
        exit(1);
    }
...
void *perform_io() {
    ...
    while (!shouldQuit()) {
        ...
        if (reqflag[cur_idx] == READ_IO){
            ...
            submitTask(hedging_pool, hedging_thread_function, cur_idx);   // hedging
            target_device = linnos_algo((long)reqflag[cur_idx], (long)reqsize[cur_idx]);  // linnos inference to pick target device
        } 
        ...
        // now, update the linnos if it is a READ_IO IO goes to the original device
        if (reqflag[cur_idx] == READ_IO && target_device == original_device_index){
            // input: long current_io_latency, long current_io_size, uint32_t device
            long update_size = ((reqsize[cur_idx] / block_size) / 512 + 7) / 8;        // align with the transform in training part.
            update_linnos(lat, update_size);   // (elaps + 1) to avoid divided by 0
        } 
...
void do_replay(void) {
    ...
    for (t = 0; t < nr_workers; t++) {
        if (pthread_join(tid[t], NULL) != 0) {
            printf("Thread joins error!\n");
            exit(1);
        }
    }
    pthread_join(track_thread, NULL);  // progress
    printf("Finish joining the threads.\n");
...
int main(int argc, char **argv) {
    ...
    if (argc != 7) {
        printf("Usage: ./io_replayer $original_device_index $devices_list $hedging_latency $trace $output_file $duration\n");
        exit(1);
    } else {
