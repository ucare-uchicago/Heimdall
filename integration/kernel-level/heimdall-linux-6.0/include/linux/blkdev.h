...
struct blk_independent_access_ranges {
	...
	#define HIS_IO_QSIZE 3

// #define LEN_PAD_LATENCY 5
struct ml_io_info {
	unsigned int 		nr_io_4k_ss[NR_IO_TYPES];	/* The snapshot of 4k IOs in queue */
	unsigned int 		nr_io_ss[NR_IO_TYPES];
	unsigned int 		nr_prev_queue_len_ss[NR_IO_TYPES];
...
struct request_queue {
	...
	unsigned int 		nr_io[NR_IO_TYPES];
	unsigned int 		nr_queue_len[NR_IO_TYPES];
	struct ml_io_info 	his_io_queue[HIS_IO_QSIZE];
	unsigned int 		his_q_index;
	spinlock_t			his_lock;
#endif
...
void his_queue_io4k_add_ss(unsigned int op, long nr_io4ks, struct request_queue *q, 
	struct bio *bio);
void his_queue_io_add_ss(unsigned int op, long nr_io, struct request_queue *q, struct bio *bio);
void his_queue_len_add_ss(unsigned int op, long nr_queue_len, struct request_queue *q, struct bio *bio); 
