...
void inline his_queue_io_add_ss(unsigned int op, long nr_io, struct request_queue *q, 
		struct bio *bio) {
	unsigned long irqflags;
	if (q && (op>=0) && (op<NR_IO_TYPES)) {
		//spin_lock_irq(&(q->his_lock));
		...
		bio->bi_nr_io[0] = 0;
		bio->bi_nr_io[1] = 0;
		bio->bi_nr_io[op] = nr_io;    // size of this IO, in byte
		//spin_unlock_irq(&(q->his_lock));
		spin_unlock_irqrestore(&(q->his_lock), irqflags);
	}
}

void inline his_queue_len_add_ss(unsigned int op, long nr_queue_len, struct request_queue *q, 
		struct bio *bio) {
	unsigned long irqflags;
	if (q && (op>=0) && (op<NR_IO_TYPES)) {
		//spin_lock_irq(&(q->his_lock));
		spin_lock_irqsave(&(q->his_lock), irqflags);
		(q->nr_queue_len)[op] += nr_queue_len;
		bio->nr_queue_len[0] = q->nr_queue_len[0];
		bio->nr_queue_len[1] = q->nr_queue_len[1];
		//spin_unlock_irq(&(q->his_lock));
		spin_unlock_irqrestore(&(q->his_lock), irqflags);
	}
}
...
void submit_bio_noacct(struct bio *bio)
{
	...
	if (sysctl_lake_enable_linnos && q->ml_enabled && q->predictor!=0 && !current->bio_list) {
		...
		long sector_size = queue_physical_block_size(bdev->bd_disk->queue);
		...
		// only inference on read IO
		if (__op == 0) {  // && TARGET_PRIO != bio_prio(bio)) {
		//if (__op == 0 && TARGET_PRIO != bio_prio(bio)) {
			unsigned int FeatNum=12;
			long feat_v6[FeatNum];
			unsigned int len_vec = FeatNum*sizeof(long)/sizeof(char);
			char feature_vec[len_vec+1];
			...
			feat_v6[0]=(long)!__op;            // in heimdall model, 1 means read IO and 0 means write IO, need to flip here
			feat_v6[1]=(long)__secs * sector_size;    // align the unit to bytes
			feat_v6[2]=(long)(q->nr_queue_len[0]+q->nr_queue_len[1]);  //queue len includes read + write IOs

			/* append historical data to input feature vector */
			__index = q->his_q_index;
			unsigned long lat;
			for (i=0; i<HIS_IO_QSIZE; i++) {
				// 1. append previous queue length (include both read and write IOs)
				feat_v6[3+i]=(long)(q->his_io_queue[(__index-i)%HIS_IO_QSIZE].nr_prev_queue_len_ss[0] + q->his_io_queue[(__index-i)%HIS_IO_QSIZE].nr_prev_queue_len_ss[1]);

				// 2. append previous latency
				lat = q->his_io_queue[(__index-i)%HIS_IO_QSIZE].latency;
				feat_v6[3+HIS_IO_QSIZE+i] = (long)(lat/1000); // us

				// 3. append previous throughput
				// unit: bytes / us
				feat_v6[3+2*HIS_IO_QSIZE+i] = (lat == 0) ? 99999999 : (long)((q->his_io_queue[(__index-i)%HIS_IO_QSIZE].nr_io_ss[0] + q->his_io_queue[(__index-i)%HIS_IO_QSIZE].nr_io_ss[1])/(lat/1000));
			}
			...
			if (likely(__sec_off > 0)) { 
				...
				//true means reject
				bio->bi_ebusy = q->predictor((char *)feat_v6, 1, weights);
				if(unlikely(sysctl_lake_linnos_debug == 2))
		...
		// if the io is not rejected, append to the queue
		his_queue_io_add_ss(__op, __secs * sector_size , q, bio);     // append the io size in byte.
		his_queue_len_add_ss(__op, 1 , q, bio);    // add the queue length by 1.
	}
#endif
