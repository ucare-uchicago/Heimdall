...
void bio_endio(struct bio *bio)
{
	...
	if (sysctl_lake_enable_linnos && bio->bi_bdev) {
		...
		if (q->ml_enabled && bio->bi_sec_size) {
			...
			/* record both read & write op*/
			if (!bio->bi_ebusy) { //lets try not processing failed ones  //XXXX
				...
				// decrease the queue len by 1.
				if (q->predictor!=0) {
					if ((q->nr_queue_len)[bio->bi_op_type] > 0) {
						(q->nr_queue_len)[bio->bi_op_type] -= 1;     // since this IO ends, reduce the queue length by 1
					}
				}

				q->his_io_queue[q->his_q_index] = (struct ml_io_info){
					.nr_io_4k_ss = {bio->bi_nr_io4k[0], bio->bi_nr_io4k[1]},
					.nr_io_ss = {bio->bi_nr_io[0], bio->bi_nr_io[1]},                        // append IO size in byte.
					.sec_size = bio->bi_sec_size,
					.sectors = bio->bi_sec_off,
					.latency = __lat,                                                        // append IO latency in ns.
					.nr_prev_queue_len_ss = {bio->nr_queue_len[0], bio->nr_queue_len[1]}     // append previous read IO(0) queue length and write IO(1) queue len.
				};
				...
				__p_pending = q->his_io_queue[q->his_q_index].pad_pending;
				...
				__p_latency = q->his_io_queue[q->his_q_index].pad_latency;
				his_queue_padding(__p_latency, __lat_us, LEN_PAD_LATENCY);
				__p_latency[LEN_PAD_LATENCY] = '\0';

				q->his_q_index = his_q_index_inc((q->his_q_index), 1);
