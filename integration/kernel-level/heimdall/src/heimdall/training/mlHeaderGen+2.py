...
def generate_1d_var_T_norm_range(v_name, v_type, power, thres, input_path):
    ...
    str_list = []
    for row in raw_array:
        str_list.append(','.join(str(round((2**power / float(item))) if float(item) != 0 else 0) for item in row)+'\n')
        # str_list.append(','.join(str(float(item)) for item in row)+'\n')
    v_literal += ','.join(item for item in str_list)
    v_literal += '};\n'
...
digits = 3
Type='long'
generate_1d_var_T('weight_0_T_'+drive, Type, 0, None, w0_path) # norm
generate_1d_var_T('weight_1_T_'+drive, Type, digits, None, w1_path)
generate_1d_var_T_norm_range('bias_0_'+drive, Type, 30, None, b0_path)     # norm - range of maxmin-scaler
generate_1d_var_T('bias_1_'+drive, Type, digits, None, b1_path)
